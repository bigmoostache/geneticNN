...
class ModelSkeleton:
    def __init__(self, inputs, outputs, submodels, runs):
    ...
    def find_inputs(self):
        ...
        if not self.check_graph():
            raise Exception("The graph is not well constructed: it has edges referring to non-existing submodel")
    ...
    def is_connected_to_output(self, run):
        ...
        for key in self.outputs:
            outputting_runs.append(self.outputs[key]['run_id'])
    ...
    def reorder_runs(self):
        ...
        for run in self.runs:
            for input in run['inputs']:
                if run['inputs'][input][0] != -1:
                    run['inputs'][input][0] = inverter[run['inputs'][input][0]]
        for output in self.outputs:
            self.outputs[output]['run_id'] = inverter[self.outputs[output]['run_id']]
